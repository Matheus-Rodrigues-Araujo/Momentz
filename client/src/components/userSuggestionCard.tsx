...
export const UserSuggestionCard = ({user}:IUserSuggestion) =>{
    ...
    return (
        <div className="flex items-center justify-around" >
            ...
            {loading ? 
                <Skeleton width={80} height={10} className=' h-10 rounded-full mr-2 product-image' style={{borderRadius: '2em'}} />
                :<h4 className="text-white font-medium" >{user.username}</h4>
            }
            {loading ? 
                <Skeleton width={120} height={10} className=' h-10 rounded-full mr-2 product-image' style={{borderRadius: '2em'}} />
                :<p className="text-customLighterPink text-thin text-sm" >New on Momentz</p>
            }
            </div>
            {loading ? 
                <Skeleton width={40} height={20} className=' h-10 rounded-full mr-2 product-image' style={{borderRadius: '2em'}} />
